...
### **Step 4.5 - Test your lambda function offline**

Before deploying our lambda function it would be wise to test it offline. We can make use of a handy plugin named `serverless-offline` that boots our lambda functions offline.

To install it we have to run the following command:

```bash
yarn install -D serverless-offline
```

we’ll have to add the configs into the `serverless.yml` file like this:
...
To finally boot the lambda run the following command:
